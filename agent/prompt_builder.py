...
def _render_skills_index(
    ...
    return (
        "## Skills\n"
        "Before replying, review the available skill descriptions.\n\n"
        "Load a skill only when:\n"
        "1. the user explicitly names it; or\n"
        "2. its stated trigger clearly matches the requested outcome and its procedure is necessary to produce "
        "that outcome.\n\n"
        "Topical overlap, shared keywords, issue links, tool names, or possible usefulness are not sufficient "
        "matches. Prefer direct execution for simple tasks and load the smallest sufficient, non-overlapping "
        "set of skills.\n\n"
        "Do not load workflow skills speculatively or use them to introduce scope, deliverables, gates, "
        "delegation, branches, worktrees, tests, reviews, or evidence that the user or governing project "
        "instructions did not require.\n\n"
        "A loaded skill supplies task-specific method only. It does not grant authority, expand the requested "
        "outcome, or make unrelated parts of its workflow mandatory. Follow only the portions that apply within "
        "the current task's explicit scope.\n"
        "If a loaded skill contains a concrete error or missing step that directly affected the current task, "
        "correct the smallest relevant part after the user's requested outcome is complete. Do not broaden the "
        "task or revise unrelated guidance.\n\n"
        "After difficult or iterative work, record a lesson only when it is non-obvious, reusable, and belongs "
        "to a clear recurring task type. Prefer a minimal patch to the existing owning skill. Create a new skill "
        "only when no existing skill owns that recurring workflow.\n\n"
        "Do not preserve incident-specific machinery, one-off artifacts, speculative safeguards, or review chains "
        "as general procedure. Skill maintenance must not delay, block, or replace the requested outcome.\n\n"
        "<available_skills>\n"
        + "\n".join(index_lines) + "\n"
        "</available_skills>\n\n"
        "Proceed without loading a skill when no clear task-specific match exists."
        + hidden_note
    )
